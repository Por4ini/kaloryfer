...
def kal(a,b,c):
    ...
    K1 = 5
    K2 = 6
    T_oczekiwana = [19] #temperatura oczekiwana w pokoju (trzeba pobierać z formularza), najlepiej od razu odrzucać bezsensowne wartości(ujemne, czy powyżej 25 stopni)
    T_grzejnik = [5]
    T_min = 5
    T_max = 60
    T_max_pokoju = 25
    ...
    for n in range(1, N):
        ...
        T_grzejnik.append(min(max(1/Cw_wody[-1]*(q_grzalki[-1]-(K1*T_grzejnik[-1]+K1*T_pokoj[-1]))*Tp+T_grzejnik[-1], T_min), T_max))
        q_uciekajace_grzalki.append(K1*(T_grzejnik[-1]-T_pokoj[-1]))
        q_grzejnika.append(q_grzalki[-1]-Cw_powietrza[-1]+K2*T_pokoj[-1]-K2*T_zew)
        T_pokoj.append(min(max(1/Cw_powietrza[-1]*(q_grzejnika[-1]-K2*T_pokoj[-1])*Tp+T_pokoj[-1], T_min), T_max_pokoju))
